...
backend_target_pairs = [ #here we have backend and target pairs. simply it means that we have 5 different combinations of backend and target and they are used to run the model on different devices.
...
parser.add_argument('--backend_target', '-bt', type=int, default=0,
                    ...
                    '''.format(*[x for x in range(len(backend_target_pairs))])) #here we have only one option for backend and target and that is used to run the model on CPU
args = parser.parse_args()

def visualize(image, det_res, fer_res, box_color=(0, 255, 0), text_color=(0, 0, 255)): #this function is used to draw the bounding box and the facial expression on the image
...
def process(detect_model, fer_model, frame): #this function is used to run the model on the input image and get the results
    h, w, _ = frame.shape
